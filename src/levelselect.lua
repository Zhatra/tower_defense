...
-- World-map node positions for 3 levels
local NODES = {
    {x=240,  y=520, label="I"},
    {x=584,  y=260, label="II"},
    {x=940,  y=470, label="III"},
}
local NODE_R = 44

function LevelSelect.new(slotData)
    local self = setmetatable({}, LevelSelect)
    self.slotData  = slotData   -- may be nil
    self.fonts = {
        title  = love.graphics.newFont(34),
        node   = love.graphics.newFont(22),
        label  = love.graphics.newFont(14),
...
function LevelSelect:draw()
    ...
    love.graphics.printf("SELECCIONAR NIVEL", 0, 40, W, "center")

    -- Draw connecting roads between nodes
    ...
    for i, nd in ipairs(NODES) do
        ...
        -- Outer glow for available levels
        ...
        if done then
            ...
            love.graphics.printf("✓", nd.x - NODE_R, nd.y - 12, NODE_R*2, "center")
        elseif unlk then
            love.graphics.setColor(hov and {1,0.92,0.70} or {0.82,0.88,1.00})
            love.graphics.printf(nd.label, nd.x - NODE_R, nd.y - 12, NODE_R*2, "center")
        else
        ...
        love.graphics.printf(Levels[i].name, nd.x - 80, nd.y + NODE_R + 8, 160, "center")

        -- Waves info
        if unlk then
            love.graphics.setColor(0.50, 0.50, 0.50)
            love.graphics.printf(#Levels[i].waves .. " oleadas", nd.x-80, nd.y+NODE_R+26, 160, "center")
        end
    end
...
-- Returns levelIndex (1/2/3) | "back" | nil
function LevelSelect:click(mx, my)
    ...
    for i, nd in ipairs(NODES) do
        if isUnlocked(self.slotData, i) then
